...
"""
Problem 1 : to display a sample of images
"""
# for letter in train_folders:
#   for imgname in sorted(os.listdir(letter))[:4]:
#     img = mpimg.imread(os.path.join(letter, imgname))
#     plt.imshow(img)
#     plt.show()
"""
"""
...
#     train_dataset.append(pickle.load(f))
#     data_len.append(train_dataset[i].shape[0])
...
"""
Problem 5 : By construction, this dataset might contain a lot of overlapping samples, 
...
in an environment where there is never an overlap, but are actually ok if you expect to see 
...
"""
Problem 6 : Let's get an idea of what an off-the-shelf classifier can give you on this data. 
...
train_features = train_dataset.reshape(n, (width*height))

n, width, height = test_dataset.shape
test_features = test_dataset.reshape(n, (width*height))

clf = LogisticRegression(solver="lbfgs", multi_class="multinomial", max_iter=1000, random_state=42, verbose=1)
